...
export class TasksKanbanView {
    ...
    getAssigneeInitials(assigneeId: string | null): string {
        if (!assigneeId || assigneeId === '00000000-0000-0000-0000-000000000000') return 'U';
        const member = this.workspaceMembers().find(m => m.memberId === assigneeId);
        if (!member) return 'U';
        const first = member.firstName ? member.firstName.charAt(0).toUpperCase() : '';
